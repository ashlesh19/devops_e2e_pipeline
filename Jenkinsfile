pipeline {
    ...
    stages {
        ...
        stage("Trigger CD pipeline") {
            steps {
                script {
                    sh "curl -v -k --user admin:${JENKINS_API_TOKEN} -X POST -H 'cache-control: no-cache' -H 'content-type: application/x-www-form-urlencoded' --data 'IMAGE_TAG=${IMAGE_TAG}' 'http://3.95.83.24/devops-manifest-pipeline/buildWithParameters?token=manifest-token'"
                }
            }
